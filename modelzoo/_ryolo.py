...
from utility.loss import GeneralLoss_fix
from utility.test_loss import GeneralLoss_fix_R, GeneralLoss_fix_RM
...
class RYoloX(BaseODModel):
    '''4 + 1 + classes'''
    def __init__(self, config, backbone, neck, head):
        super(RYoloX, self).__init__()
        self.config = config
    ...
    def set(self, args, device):
        ...
        self.loss = GeneralLoss_fix_R(self.config, device, reduction='mean')

    def training_loss(self, sample):
        dt = self.core(sample['imgs']) #[B, 4+1+cls, n_anchors_all]
        num_of_class = dt.shape[1]-6
        cls_dt = dt[:, 6:, :].clone()  #[B, cls, n_anchors_all]
        obj_dt = dt[:, 4, :].clone()   #[B, n_anchors_all]
        obj_aux_dt = dt[:, 5, :].clone()   #[B, n_anchors_all]
        ...
        dt['aux_obj'] = obj_aux_dt.view(-1)[effective_mask]
...
class RYoloX_wa(BaseODModel):
    '''4 + 1 + classes'''
    def __init__(self, config, backbone, neck, head):
        super(RYoloX_wa, self).__init__()
        self.config = config
        self.backbone = backbone
        self.neck = neck
        self.head = head
...
class RYoloX_B(BaseODModel):
    '''4 + 1 + classes'''
    def __init__(self, config, backbone, neck, head):
        super(RYoloX_B, self).__init__()
        self.config = config
        self.backbone = backbone
        self.neck = neck
        self.head = head
    ...
    def core(self,input):
        fms, mask_dt = self.backbone(input)
        rea_fms = self.neck(*fms)
        dt,  mask_gt = self.head(*rea_fms)
        return dt, [mask_dt, mask_gt]
    ...
    def set(self, args, device):
        ...
        assert True in ['iou' in loss for loss in self.config.loss.reg_type], 'Please adding iou-type loss'
        self.loss = GeneralLoss_fix_RM(self.config, device, reduction='mean')

    def training_loss(self, sample):
        dt, masks = self.core(sample['imgs']) #[B, 4+1+cls, n_anchors_all]
        num_of_class = dt.shape[1]-6
        cls_dt = dt[:, 6:, :].clone()  #[B, cls, n_anchors_all]
        obj_dt = dt[:, 4, :].clone()   #[B, n_anchors_all]
        obj_aux_dt = dt[:, 5, :].clone()   #[B, n_anchors_all]
