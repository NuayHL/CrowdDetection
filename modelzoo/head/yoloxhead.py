...
@HeadRegister.register
@HeadRegister.register('yolox_rhead')
class YOLOX_Rhead(nn.Module):
    def __init__(self, classes, anchors_per_grid, p3_channels=256, depthwise=False, act='silu'):
        super(YOLOX_Rhead, self).__init__()
        self.depthwise = depthwise
        width = p3_channels/256.0
        self.act = act
        self.classes = classes
        ...
        self.mask_attention = Conv_Mask_2D(5)
    ...
    def mask_gen(self, *p):
        ...
        for id, layer in enumerate(p):
            ...
            reg_f = self.reg_conv[id](reg_x)
            obj_out = self.obj_pred[id](reg_f)
            mask_attention = self.mask_attention(self.sigmoid(obj_out))
            aux_obj_pre.append(obj_out)
            masks.append(mask_attention)
        return masks, aux_obj_pre

    def forward(self, *p):
        output = []
        masks, aux_obj_pre = self.mask_gen(*p)  # need test torch.no_grad()
        for id, layer in enumerate(p):
            x = self.stems[id](layer)
            cls_x = x
            reg_x = x
