...
class Yolov3(nn.Module):
    ...
    def __init__(self, config, backbone, neck, head):
        ...
        self.anchor = Anchor(self.config)
    ...
    def set(self, args, device):
        ...
        if self.use_anchor:
            self.anchs = torch.from_numpy(self.anchor.gen_Bbox(singleBatch=True)).float().to(device)
        else:
            self.anchs = torch.from_numpy(self.anchor.gen_points(singleBatch=True)).float().to(device)
