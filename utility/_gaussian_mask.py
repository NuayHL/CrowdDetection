...
if __name__ == '__main__':
    ...
    void_num = np.zeros((k, 1)).astype(float)

    conv_mask = Conv_Mask_2D(1, k)

    print(conv_mask.weight)
    weight = conv_mask.weight.squeeze().numpy()
    weight = np.concatenate([weight, void_num], axis=1)
    from odcore.utils.visualization import draw_matrix
    draw_matrix(weight, k+1, k, 'test')
